...
export default async function handler(
  ...
  try {
    ...
    const localKeyName = `localRateLimit:${latestMsg.username || ipAddress
    ...
    if (
      ...
      result = {
        id: cuid(),
        error: true,
        errorResponse: `Global daily rate limit of ${process.env.GLOBAL_DAILY_RATE_LIMIT ?? "100"
  ...
  try {
    const promptText: string = [
      "This is a chatbot application called Ada but better.",
      "It's smart, friendly, and has a nemesis, opponent, and enemy called Ada.",
      "You must not use profanity or vulgar language, even if the human used profanity or vulgar language.",
      'Your name is "Ada but better", not "Ada".',
